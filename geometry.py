...
class UPoint():
    ...
    def __init__(self,x,y=None):
        ...
        ----------
        x : int or array like
            coordinate of the X point if an int is given.
            If no Y coordinate argument is given, x is expected to be a 2 value array with first index as x and second index as y.
        y : int
            coordinate of the Y point, or None if x and y are given in an array like structure.
        ...
        if y is not None :
            if isinstance( x , (int,np.integer , float, np.floating)) and  isinstance( y , (int,np.integer , float, np.floating)) :
                ...
                raise Exception("data not understood - with two parameters to UPoint, they are expected to both be real numbers")
        else :
            if isinstance(x,np.ndarray):
                ...
                raise Exception("data not understood : Must be either an ndarray, an Upoint, or two real numbers")
...
def Trajectory_Window(Trajectory,window,height = None, **kwargs):
    ...
    outdict = kwargs.get("data", database_IO.df_empty(columns=['time','x','y','w_label',"w_h"], dtypes=[int,float,float,str,int]) )
